...
subprocess.run(['supabase','db','query','--linked','--project-ref','fdfhyekuehybjkfyatjn',f"update public.ts_responses set started_at=now()-interval '3 minutes' where id='{r['id']}';"],check=True,stdout=subprocess.DEVNULL)
o=io.BytesIO()
...
q2=call('save_question',owner=owner,classId=c['id'],mode='answer',prompt='取消測試',rubric='不應評分',position=1)
call('open_question',owner=owner,classId=c['id'],id=q2['id'])
r3=call('start',code=c['code'],token=token,questionId=q2['id'])
subprocess.run(['supabase','db','query','--linked','--project-ref','fdfhyekuehybjkfyatjn',f"update public.ts_responses set status='failed' where id='{r3['id']}';"],check=True,stdout=subprocess.DEVNULL)
ok('取消'in call('submit',code=c['code'],token=token,questionId=q2['id'],audio=base64.b64encode(o.getvalue()).decode()).get('error',''),'canceled recording is rejected, not reported as received')
